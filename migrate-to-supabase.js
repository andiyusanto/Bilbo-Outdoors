  ...
  try {
    ...
    for (const order of orders) {
      ...
      if (existing.rows.length === 0) {
        await client.query(
          `INSERT INTO orders (id, customer_name, customer_whatsapp, start_date, end_date, rent_duration, total_price, id_card_base64, status, created_at, late_days, late_fee, confirmation_token, returned_at, pickup_id_type, amount_paid, status_history)
          ...
          [
            ...
            order.statusHistory ? JSON.stringify(order.statusHistory) : null
          ]
        );
    ...
    if (settings) {
      ...
      await client.query(
        ...
        [Number(settings.lateToleranceHours), settings.pendingExpiryHours ?? null, JSON.stringify(settings.operatingHours), settings.footer ? JSON.stringify(settings.footer) : null, settings.runningText ?? null]
      );
