...
export default function OrderResiPrint({ order, products, storeFooter }: OrderResiPrintProps) {
  ...
  return (
    <div id="print-resi" className="hidden print:block p-8 text-black bg-white text-xs leading-relaxed">
      <div className="flex items-center justify-between border-b-2 border-black pb-4 mb-4">
        ...
        <div className="text-right">
          ...
          <p className="font-mono">No: {order.id}</p>
          <p className="font-mono text-[10px] text-zinc-600">Dicetak: {formatDateTimeLabel(new Date())}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4 border-2 border-black p-3">
        <div>
          <p className="font-black uppercase text-[10px] text-zinc-500">Penyewa</p>
          <p className="font-black">{order.customerName}</p>
          <p>{order.customerWhatsApp}</p>
        </div>
        <div className="text-right">
          <p className="font-black uppercase text-[10px] text-zinc-500">Status</p>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4 border-2 border-black p-3">
        <div>
          <p className="font-black uppercase text-[10px] text-zinc-500">Jadwal Sewa</p>
          <p className="font-black">{formatDateLabel(order.startDate)} s/d {formatDateLabel(order.endDate)}</p>
        </div>
        <div className="text-right">
          <p className="font-black uppercase text-[10px] text-zinc-500">Total Durasi</p>
      ...
      <table className="w-full border-collapse mb-4">
        <thead>
          <tr className="border-b-2 border-black text-[10px] uppercase text-zinc-600">
            ...
            return (
              <tr key={index} className="border-b border-zinc-300">
                <td className="py-1.5">
                  ...
                  {attrLine && <p className="text-[10px] text-zinc-600">{attrLine}</p>}
                </td>
                <td className="text-right py-1.5">{item.quantity}</td>
                <td className="text-right py-1.5 font-mono">{priceLabel}</td>
              </tr>
            );
          })}
        </tbody>
      ...
      <div className="border-2 border-black p-3 mb-6 space-y-1.5">
        ...
        <div className="flex justify-between border-t-2 border-black pt-1.5 font-black text-sm">
          <span>Total Invoice</span>
          <span className="font-mono">Rp {totalInvoice.toLocaleString('id-ID')}</span>
        </div>
        {order.pickupIdType && (
          <p className="text-[10px] text-zinc-600 pt-1">Jaminan diberikan saat pengambilan: {order.pickupIdType}</p>
        )}
      </div>

      <div className="grid grid-cols-2 gap-8 mt-12 text-center text-[10px]">
        <div>
